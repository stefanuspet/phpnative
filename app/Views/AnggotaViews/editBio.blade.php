...
@section('content')
<div class="w-full">
    <h1 class="text-4xl font-bold pb-10 text-blue-950">Edit Profile</h1>
    <div class="shadow-md rounded-md bg-white w-4/5">
        <form action="/dashboard/anggota/update/{{$anggota->nid}}" class="p-4" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="id_dojo" id="id_dojo" class="w-full border border-blue-600 rounded-md p-2" required disabled>
                    <option value="">Choose</option>
                    @if($anggota->dojo)
                    <option selected value="{{ $anggota->id_dojo }}">{{ $anggota->dojo->nama }}</option>
                    @endif
                </select>
                <input type="hidden" name="id_dojo" value="{{$anggota->dojo->id}}">
            </div>
            <p>{{$anggota->id_dojo}}</p>
            <div class="mb-3">
