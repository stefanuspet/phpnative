<aside class="bg-blue-900 px-4">
    <div class="py-10 cursor-pointer w-full inline-flex justify-end">
        <svg id="humburger-menu" class="w-8" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
            <g id="SVGRepo_iconCarrier">
                <path d="M4 6H20M4 12H20M4 18H20" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
            </g>
        </svg>
        <svg id="cross-menu" class="w-8 text-white hidden" fill="currentColor" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
            <g id="SVGRepo_iconCarrier">
                <path d="M18.8,16l5.5-5.5c0.8-0.8,0.8-2,0-2.8l0,0C24,7.3,23.5,7,23,7c-0.5,0-1,0.2-1.4,0.6L16,13.2l-5.5-5.5 c-0.8-0.8-2.1-0.8-2.8,0C7.3,8,7,8.5,7,9.1s0.2,1,0.6,1.4l5.5,5.5l-5.5,5.5C7.3,21.9,7,22.4,7,23c0,0.5,0.2,1,0.6,1.4 C8,24.8,8.5,25,9,25c0.5,0,1-0.2,1.4-0.6l5.5-5.5l5.5,5.5c0.8,0.8,2.1,0.8,2.8,0c0.8-0.8,0.8-2.1,0-2.8L18.8,16z"></path>
            </g>
        </svg>
    </div>
    <div id="nav-close" class="w-full">
        <a href="/dashboard-majelis" class="flex items-center justify-center gap-3 bg-slate-100 bg-opacity-50 w-8 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" viewBox="0 0 64 64" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path d="M55.64 22.751H35.09C34.5596 22.751 34.0509 22.9617 33.6758 23.3368C33.3007 23.7118 33.09 24.2205 33.09 24.751V55.571C33.0916 56.1009 33.3028 56.6087 33.6775 56.9834C34.0523 57.3582 34.5601 57.5694 35.09 57.571H55.64C56.1699 57.5694 56.6777 57.3582 57.0525 56.9834C57.4272 56.6087 57.6384 56.1009 57.64 55.571V24.751C57.64 24.2205 57.4293 23.7118 57.0542 23.3368C56.6791 22.9617 56.1704 22.751 55.64 22.751Z" fill="currentColor"></path>
                        <path d="M55.64 5.62695H35.09C34.5596 5.62695 34.0509 5.83767 33.6758 6.21274C33.3007 6.58781 33.09 7.09652 33.09 7.62695V17.8969C33.0916 18.4269 33.3028 18.9347 33.6775 19.3094C34.0523 19.6841 34.5601 19.8954 35.09 19.8969H55.64C56.1699 19.8954 56.6777 19.6841 57.0525 19.3094C57.4272 18.9347 57.6384 18.4269 57.64 17.8969V7.62695C57.64 7.09652 57.4293 6.58781 57.0542 6.21274C56.6791 5.83767 56.1704 5.62695 55.64 5.62695Z" fill="currentColor"></path>
                        <path d="M28.24 36.451H7.7C6.59543 36.451 5.7 37.3465 5.7 38.451V55.5711C5.7 56.6756 6.59543 57.5711 7.7 57.5711H28.24C29.3446 57.5711 30.24 56.6756 30.24 55.5711V38.451C30.24 37.3465 29.3446 36.451 28.24 36.451Z" fill="currentColor"></path>
                        <path d="M28.24 5.62697H7.70002C7.43712 5.62604 7.17661 5.67714 6.93355 5.77733C6.69048 5.87751 6.46964 6.02477 6.28373 6.21068C6.09783 6.39658 5.95054 6.61742 5.85035 6.86049C5.75017 7.10355 5.6991 7.36406 5.70002 7.62697V31.557C5.70002 32.0874 5.91074 32.5961 6.28581 32.9712C6.66088 33.3462 7.16959 33.557 7.70002 33.557H28.24C28.7704 33.557 29.2791 33.3462 29.6542 32.9712C30.0293 32.5961 30.24 32.0874 30.24 31.557V7.62697C30.2409 7.36406 30.1898 7.10355 30.0896 6.86049C29.9895 6.61742 29.8422 6.39658 29.6563 6.21068C29.4704 6.02477 29.2495 5.87751 29.0065 5.77733C28.7634 5.67714 28.5029 5.62604 28.24 5.62697Z" fill="currentColor"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 hidden font-semibold">Profile</p>
        </a>
        <a href="/dashboard-majelis/cabang" class="flex items-center justify-center gap-3 bg-slate-100 bg-opacity-50 w-8 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" fill="currentColor" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 14 14" id="svg2">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <metadata id="metadata8">
                            <rdf:rdf>
                                <cc:work rdf:about="">
                                    <dc:format>image/svg+xml</dc:format>
                                    <dc:type rdf:resource="http://purl.org/dc/dcmitype/StillImage"></dc:type>
                                    <dc:title></dc:title>
                                </cc:work>
                            </rdf:rdf>
                        </metadata>
                        <defs id="defs6"></defs>
                        <rect width="14" height="14" x="0" y="0" id="canvas" style="fill:none;stroke:none;visibility:hidden"></rect>
                        <path d="M 7,0 C 6.75,0.0032 6.5,0.1644239 6.5,0.5 L 6.5,4.375 1,7 13,7 7.5,4.375 7.5,0.5 C 7.5,0.1516409 7.25,-0.0031957 7,0 z M 8,0 8,3 12,3 10,1.5 12,0 z m -7,8 0,1 1,0 0,4 -1,0 0,1 12,0 0,-1 -1,0 0,-4 1,0 0,-1 z m 6,1.5 c 1,0 2,0.5 2,1.5 l 0,2 -4,0 0,-2 C 5,10 6,9.5 7,9.5 z" id="town-hall" style="fill-opacity:1;stroke:none"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 hidden font-semibold">Cabang</p>
        </a>
        <a href="/dashboard-majelis/kegiatan">
            <div class="w-8 h-8 rounded-full bg-slate-100 bg-opacity-50 mb-3 hover:bg-opacity-100 flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M16.53 11.06L15.47 10l-4.88 4.88-2.12-2.12-1.06 1.06L10.59 17l5.94-5.94zM19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11z"></path>
                    </g>
                </svg>
            </div>
        </a>
        <a href="/dashboard-majelis/anggota">
            <div class="w-8 h-8 rounded-full bg-slate-100 bg-opacity-50 mb-3 hover:bg-opacity-100 flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" fill="currentColor" viewBox="-3 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <title>group</title>
                        <path d="M20.906 20.75c1.313 0.719 2.063 2 1.969 3.281-0.063 0.781-0.094 0.813-1.094 0.938-0.625 0.094-4.563 0.125-8.625 0.125-4.594 0-9.406-0.094-9.75-0.188-1.375-0.344-0.625-2.844 1.188-4.031 1.406-0.906 4.281-2.281 5.063-2.438 1.063-0.219 1.188-0.875 0-3-0.281-0.469-0.594-1.906-0.625-3.406-0.031-2.438 0.438-4.094 2.563-4.906 0.438-0.156 0.875-0.219 1.281-0.219 1.406 0 2.719 0.781 3.25 1.938 0.781 1.531 0.469 5.625-0.344 7.094-0.938 1.656-0.844 2.188 0.188 2.469 0.688 0.188 2.813 1.188 4.938 2.344zM3.906 19.813c-0.5 0.344-0.969 0.781-1.344 1.219-1.188 0-2.094-0.031-2.188-0.063-0.781-0.188-0.344-1.625 0.688-2.25 0.781-0.5 2.375-1.281 2.813-1.375 0.563-0.125 0.688-0.469 0-1.656-0.156-0.25-0.344-1.063-0.344-1.906-0.031-1.375 0.25-2.313 1.438-2.719 1-0.375 2.125 0.094 2.531 0.938 0.406 0.875 0.188 3.125-0.25 3.938-0.5 0.969-0.406 1.219 0.156 1.375 0.125 0.031 0.375 0.156 0.719 0.313-1.375 0.563-3.25 1.594-4.219 2.188zM24.469 18.625c0.75 0.406 1.156 1.094 1.094 1.813-0.031 0.438-0.031 0.469-0.594 0.531-0.156 0.031-0.875 0.063-1.813 0.063-0.406-0.531-0.969-1.031-1.656-1.375-1.281-0.75-2.844-1.563-4-2.063 0.313-0.125 0.594-0.219 0.719-0.25 0.594-0.125 0.688-0.469 0-1.656-0.125-0.25-0.344-1.063-0.344-1.906-0.031-1.375 0.219-2.313 1.406-2.719 1.031-0.375 2.156 0.094 2.531 0.938 0.406 0.875 0.25 3.125-0.188 3.938-0.5 0.969-0.438 1.219 0.094 1.375 0.375 0.125 1.563 0.688 2.75 1.313z"></path>
                    </g>
                </svg>
            </div>
        </a>
        <a href="/dashboard-majelis/dojoMajelis">
            <div class="w-8 h-8 rounded-full bg-slate-100 bg-opacity-50 mb-3 hover:bg-opacity-100 flex justify-center items-center">
                <svg class="text-blue-800 w-4 h-4" fill="currentColor" height="200px" width="200px" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 297 297" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 297 297">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <g>
                            <path d="m148.85,58.072c15.466,0 28.047-12.582 28.047-28.047 0-15.465-12.582-28.047-28.047-28.047-15.465,0-28.047,12.582-28.047,28.047 0,15.466 12.582,28.047 28.047,28.047z"></path>
                            <path d="m104.631,144.108h88.428c5.759,0 10.427-4.669 10.427-10.428v-38.556c0-10.673-6.859-20.137-17.001-23.46l-.047-.016-14.835-2.456c-1.263-0.388-2.612,0.28-3.066,1.526l-16.832,46.185c-0.971,2.664-4.739,2.664-5.71,0l-16.833-46.185c-0.367-1.006-1.316-1.636-2.333-1.636-0.241,0-15.567,2.564-15.567,2.564-10.226,3.407-17.058,12.887-17.058,23.606v38.429c1.42109e-14,5.759 4.668,10.427 10.427,10.427z"></path>
                            <path d="m276.738,215.266c-0.055-0.018-0.11-0.036-0.165-0.053l-8.028-2.462c-2.415-0.74-5.037-0.155-6.907,1.535l-17.307,15.647-17.307-15.647c-1.87-1.691-4.493-2.277-6.907-1.535l-8.029,2.462c-0.055,0.017-0.11,0.035-0.165,0.053-12.12,4.04-20.262,15.338-20.262,28.113v44.477c0,3.958 3.208,7.166 7.166,7.166h91.006c3.958,0 7.166-3.208 7.166-7.166v-44.477c0.001-12.775-8.142-24.073-20.261-28.113z"></path>
                            <path d="m244.331,207.808c16.496,0 29.917-13.421 29.917-29.917s-13.421-29.917-29.917-29.917c-16.497,0-29.918,13.421-29.918,29.917s13.421,29.917 29.918,29.917z"></path>
                            <path d="m85.076,215.266c-0.055-0.018-0.11-0.036-0.165-0.053l-8.028-2.462c-2.414-0.742-5.037-0.156-6.907,1.535l-17.308,15.647-17.307-15.647c-1.87-1.691-4.493-2.277-6.907-1.535 0,0-8.137,2.497-8.193,2.515-12.118,4.04-20.261,15.338-20.261,28.113v44.477c0,3.958 3.208,7.166 7.166,7.166h91.006c3.958,0 7.166-3.208 7.166-7.166v-44.477c0-12.775-8.142-24.073-20.262-28.113z"></path>
                            <path d="M22.751,177.89c0,16.496,13.421,29.917,29.917,29.917c16.497,0,29.918-13.421,29.918-29.917s-13.421-29.917-29.918-29.917 C36.172,147.973,22.751,161.394,22.751,177.89z"></path>
                            <path d="m252.506,98.996c-2.451-0.734-5.212-0.235-7.275,1.579-3.185,2.799-3.498,7.651-0.699,10.836l10.496,11.944c1.458,1.658 3.559,2.609 5.767,2.609 2.208,0 4.31-0.951 5.767-2.609l10.496-11.944c2.799-3.185 2.486-8.037-0.699-10.836-2.377-2.089-5.681-2.441-8.368-1.165-1.562-16.77-8.096-32.506-18.923-44.826-2.799-3.185-7.65-3.499-10.836-0.699-3.185,2.799-3.498,7.651-0.699,10.836 8.282,9.423 13.425,21.404 14.973,34.275z"></path>
                            <path d="m31.128,123.355c1.458,1.658 3.559,2.609 5.767,2.609 2.208,0 4.31-0.951 5.767-2.609l10.496-11.944c2.799-3.185 2.486-8.037-0.699-10.836-2.064-1.813-4.824-2.312-7.275-1.578 1.548-12.871 6.691-24.852 14.972-34.275 2.799-3.185 2.486-8.037-0.699-10.836-3.186-2.799-8.037-2.485-10.836,0.699-10.826,12.32-17.36,28.056-18.923,44.825-2.688-1.277-5.991-0.924-8.368,1.165-3.185,2.799-3.498,7.651-0.699,10.836l10.497,11.944z"></path>
                            <path d="m154.621,69.633c-0.647-0.705-1.594-1.06-2.551-1.06h-7.14c-0.957,0-1.904,0.355-2.551,1.06-1.002,1.092-1.147,2.669-0.436,3.902l3.817,5.754-1.787,15.073 3.518,9.36c0.343,0.941 1.674,0.941 2.017,0l3.518-9.36-1.787-15.073 3.817-5.754c0.712-1.233 0.567-2.81-0.435-3.902z"></path>
                        </g>
                    </g>
                </svg>
            </div>
        </a>
        <a href="/logout" class="flex items-center justify-center gap-3 bg-slate-100 bg-opacity-50 w-8 h-8 hover:bg-opacity-100 rounded-full">
            <div class="flex justify-center items-center">
                <svg viewBox="0 0 24 24" class="text-blue-800 w-5 h-5" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M16.125 12C16.125 11.5858 15.7892 11.25 15.375 11.25L4.40244 11.25L6.36309 9.56944C6.67759 9.29988 6.71401 8.8264 6.44444 8.51191C6.17488 8.19741 5.7014 8.16099 5.38691 8.43056L1.88691 11.4306C1.72067 11.573 1.625 11.7811 1.625 12C1.625 12.2189 1.72067 12.427 1.88691 12.5694L5.38691 15.5694C5.7014 15.839 6.17488 15.8026 6.44444 15.4881C6.71401 15.1736 6.67759 14.7001 6.36309 14.4306L4.40244 12.75L15.375 12.75C15.7892 12.75 16.125 12.4142 16.125 12Z" fill="#3359d7"></path>
                        <path d="M9.375 8C9.375 8.70219 9.375 9.05329 9.54351 9.3055C9.61648 9.41471 9.71025 9.50848 9.81946 9.58145C10.0717 9.74996 10.4228 9.74996 11.125 9.74996L15.375 9.74996C16.6176 9.74996 17.625 10.7573 17.625 12C17.625 13.2426 16.6176 14.25 15.375 14.25L11.125 14.25C10.4228 14.25 10.0716 14.25 9.8194 14.4185C9.71023 14.4915 9.6165 14.5852 9.54355 14.6944C9.375 14.9466 9.375 15.2977 9.375 16C9.375 18.8284 9.375 20.2426 10.2537 21.1213C11.1324 22 12.5464 22 15.3748 22L16.3748 22C19.2032 22 20.6174 22 21.4961 21.1213C22.3748 20.2426 22.3748 18.8284 22.3748 16L22.3748 8C22.3748 5.17158 22.3748 3.75736 21.4961 2.87868C20.6174 2 19.2032 2 16.3748 2L15.3748 2C12.5464 2 11.1324 2 10.2537 2.87868C9.375 3.75736 9.375 5.17157 9.375 8Z" fill="#3359d7"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 hidden font-semibold">Logout</p>
        </a>
    </div>
    <!-- second nav -->
    <div id="nav-open" class="w-full hidden">
        <a href="/dashboard-majelis" class="flex items-center justify-start px-5 gap-3 bg-slate-100 bg-opacity-50 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" viewBox="0 0 64 64" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path d="M55.64 22.751H35.09C34.5596 22.751 34.0509 22.9617 33.6758 23.3368C33.3007 23.7118 33.09 24.2205 33.09 24.751V55.571C33.0916 56.1009 33.3028 56.6087 33.6775 56.9834C34.0523 57.3582 34.5601 57.5694 35.09 57.571H55.64C56.1699 57.5694 56.6777 57.3582 57.0525 56.9834C57.4272 56.6087 57.6384 56.1009 57.64 55.571V24.751C57.64 24.2205 57.4293 23.7118 57.0542 23.3368C56.6791 22.9617 56.1704 22.751 55.64 22.751Z" fill="currentColor"></path>
                        <path d="M55.64 5.62695H35.09C34.5596 5.62695 34.0509 5.83767 33.6758 6.21274C33.3007 6.58781 33.09 7.09652 33.09 7.62695V17.8969C33.0916 18.4269 33.3028 18.9347 33.6775 19.3094C34.0523 19.6841 34.5601 19.8954 35.09 19.8969H55.64C56.1699 19.8954 56.6777 19.6841 57.0525 19.3094C57.4272 18.9347 57.6384 18.4269 57.64 17.8969V7.62695C57.64 7.09652 57.4293 6.58781 57.0542 6.21274C56.6791 5.83767 56.1704 5.62695 55.64 5.62695Z" fill="currentColor"></path>
                        <path d="M28.24 36.451H7.7C6.59543 36.451 5.7 37.3465 5.7 38.451V55.5711C5.7 56.6756 6.59543 57.5711 7.7 57.5711H28.24C29.3446 57.5711 30.24 56.6756 30.24 55.5711V38.451C30.24 37.3465 29.3446 36.451 28.24 36.451Z" fill="currentColor"></path>
                        <path d="M28.24 5.62697H7.70002C7.43712 5.62604 7.17661 5.67714 6.93355 5.77733C6.69048 5.87751 6.46964 6.02477 6.28373 6.21068C6.09783 6.39658 5.95054 6.61742 5.85035 6.86049C5.75017 7.10355 5.6991 7.36406 5.70002 7.62697V31.557C5.70002 32.0874 5.91074 32.5961 6.28581 32.9712C6.66088 33.3462 7.16959 33.557 7.70002 33.557H28.24C28.7704 33.557 29.2791 33.3462 29.6542 32.9712C30.0293 32.5961 30.24 32.0874 30.24 31.557V7.62697C30.2409 7.36406 30.1898 7.10355 30.0896 6.86049C29.9895 6.61742 29.8422 6.39658 29.6563 6.21068C29.4704 6.02477 29.2495 5.87751 29.0065 5.77733C28.7634 5.67714 28.5029 5.62604 28.24 5.62697Z" fill="currentColor"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 font-semibold">Profile</p>
        </a>
        <a href="/dashboard-majelis/cabang" class="flex items-center justify-start px-5 gap-3 bg-slate-100 bg-opacity-50 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" fill="currentColor" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 14 14" id="svg2">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <metadata id="metadata8">
                            <rdf:rdf>
                                <cc:work rdf:about="">
                                    <dc:format>image/svg+xml</dc:format>
                                    <dc:type rdf:resource="http://purl.org/dc/dcmitype/StillImage"></dc:type>
                                    <dc:title></dc:title>
                                </cc:work>
                            </rdf:rdf>
                        </metadata>
                        <defs id="defs6"></defs>
                        <rect width="14" height="14" x="0" y="0" id="canvas" style="fill:none;stroke:none;visibility:hidden"></rect>
                        <path d="M 7,0 C 6.75,0.0032 6.5,0.1644239 6.5,0.5 L 6.5,4.375 1,7 13,7 7.5,4.375 7.5,0.5 C 7.5,0.1516409 7.25,-0.0031957 7,0 z M 8,0 8,3 12,3 10,1.5 12,0 z m -7,8 0,1 1,0 0,4 -1,0 0,1 12,0 0,-1 -1,0 0,-4 1,0 0,-1 z m 6,1.5 c 1,0 2,0.5 2,1.5 l 0,2 -4,0 0,-2 C 5,10 6,9.5 7,9.5 z" id="town-hall" style="fill-opacity:1;stroke:none"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 font-semibold">Cabang</p>
        </a>
        <a href="/dashboard-majelis/kegiatan" class="flex items-center justify-start px-5 gap-3 bg-slate-100 bg-opacity-50 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M16.53 11.06L15.47 10l-4.88 4.88-2.12-2.12-1.06 1.06L10.59 17l5.94-5.94zM19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11z"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 font-semibold">Kegiatan</p>
        </a>
        <a href="/dashboard-majelis/anggota" class="flex items-center justify-start px-5 gap-3 bg-slate-100 bg-opacity-50 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-5 h-5" fill="currentColor" viewBox="-3 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <title>group</title>
                        <path d="M20.906 20.75c1.313 0.719 2.063 2 1.969 3.281-0.063 0.781-0.094 0.813-1.094 0.938-0.625 0.094-4.563 0.125-8.625 0.125-4.594 0-9.406-0.094-9.75-0.188-1.375-0.344-0.625-2.844 1.188-4.031 1.406-0.906 4.281-2.281 5.063-2.438 1.063-0.219 1.188-0.875 0-3-0.281-0.469-0.594-1.906-0.625-3.406-0.031-2.438 0.438-4.094 2.563-4.906 0.438-0.156 0.875-0.219 1.281-0.219 1.406 0 2.719 0.781 3.25 1.938 0.781 1.531 0.469 5.625-0.344 7.094-0.938 1.656-0.844 2.188 0.188 2.469 0.688 0.188 2.813 1.188 4.938 2.344zM3.906 19.813c-0.5 0.344-0.969 0.781-1.344 1.219-1.188 0-2.094-0.031-2.188-0.063-0.781-0.188-0.344-1.625 0.688-2.25 0.781-0.5 2.375-1.281 2.813-1.375 0.563-0.125 0.688-0.469 0-1.656-0.156-0.25-0.344-1.063-0.344-1.906-0.031-1.375 0.25-2.313 1.438-2.719 1-0.375 2.125 0.094 2.531 0.938 0.406 0.875 0.188 3.125-0.25 3.938-0.5 0.969-0.406 1.219 0.156 1.375 0.125 0.031 0.375 0.156 0.719 0.313-1.375 0.563-3.25 1.594-4.219 2.188zM24.469 18.625c0.75 0.406 1.156 1.094 1.094 1.813-0.031 0.438-0.031 0.469-0.594 0.531-0.156 0.031-0.875 0.063-1.813 0.063-0.406-0.531-0.969-1.031-1.656-1.375-1.281-0.75-2.844-1.563-4-2.063 0.313-0.125 0.594-0.219 0.719-0.25 0.594-0.125 0.688-0.469 0-1.656-0.125-0.25-0.344-1.063-0.344-1.906-0.031-1.375 0.219-2.313 1.406-2.719 1.031-0.375 2.156 0.094 2.531 0.938 0.406 0.875 0.25 3.125-0.188 3.938-0.5 0.969-0.438 1.219 0.094 1.375 0.375 0.125 1.563 0.688 2.75 1.313z"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 font-semibold">Kohai</p>
        </a>
        <a href="/dashboard-majelis/dojoMajelis" class="flex items-center justify-start px-5 gap-3 bg-slate-100 bg-opacity-50 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg class="text-blue-800 w-4 h-4" fill="currentColor" height="200px" width="200px" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 297 297" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 297 297">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <g>
                            <path d="m148.85,58.072c15.466,0 28.047-12.582 28.047-28.047 0-15.465-12.582-28.047-28.047-28.047-15.465,0-28.047,12.582-28.047,28.047 0,15.466 12.582,28.047 28.047,28.047z"></path>
                            <path d="m104.631,144.108h88.428c5.759,0 10.427-4.669 10.427-10.428v-38.556c0-10.673-6.859-20.137-17.001-23.46l-.047-.016-14.835-2.456c-1.263-0.388-2.612,0.28-3.066,1.526l-16.832,46.185c-0.971,2.664-4.739,2.664-5.71,0l-16.833-46.185c-0.367-1.006-1.316-1.636-2.333-1.636-0.241,0-15.567,2.564-15.567,2.564-10.226,3.407-17.058,12.887-17.058,23.606v38.429c1.42109e-14,5.759 4.668,10.427 10.427,10.427z"></path>
                            <path d="m276.738,215.266c-0.055-0.018-0.11-0.036-0.165-0.053l-8.028-2.462c-2.415-0.74-5.037-0.155-6.907,1.535l-17.307,15.647-17.307-15.647c-1.87-1.691-4.493-2.277-6.907-1.535l-8.029,2.462c-0.055,0.017-0.11,0.035-0.165,0.053-12.12,4.04-20.262,15.338-20.262,28.113v44.477c0,3.958 3.208,7.166 7.166,7.166h91.006c3.958,0 7.166-3.208 7.166-7.166v-44.477c0.001-12.775-8.142-24.073-20.261-28.113z"></path>
                            <path d="m244.331,207.808c16.496,0 29.917-13.421 29.917-29.917s-13.421-29.917-29.917-29.917c-16.497,0-29.918,13.421-29.918,29.917s13.421,29.917 29.918,29.917z"></path>
                            <path d="m85.076,215.266c-0.055-0.018-0.11-0.036-0.165-0.053l-8.028-2.462c-2.414-0.742-5.037-0.156-6.907,1.535l-17.308,15.647-17.307-15.647c-1.87-1.691-4.493-2.277-6.907-1.535 0,0-8.137,2.497-8.193,2.515-12.118,4.04-20.261,15.338-20.261,28.113v44.477c0,3.958 3.208,7.166 7.166,7.166h91.006c3.958,0 7.166-3.208 7.166-7.166v-44.477c0-12.775-8.142-24.073-20.262-28.113z"></path>
                            <path d="M22.751,177.89c0,16.496,13.421,29.917,29.917,29.917c16.497,0,29.918-13.421,29.918-29.917s-13.421-29.917-29.918-29.917 C36.172,147.973,22.751,161.394,22.751,177.89z"></path>
                            <path d="m252.506,98.996c-2.451-0.734-5.212-0.235-7.275,1.579-3.185,2.799-3.498,7.651-0.699,10.836l10.496,11.944c1.458,1.658 3.559,2.609 5.767,2.609 2.208,0 4.31-0.951 5.767-2.609l10.496-11.944c2.799-3.185 2.486-8.037-0.699-10.836-2.377-2.089-5.681-2.441-8.368-1.165-1.562-16.77-8.096-32.506-18.923-44.826-2.799-3.185-7.65-3.499-10.836-0.699-3.185,2.799-3.498,7.651-0.699,10.836 8.282,9.423 13.425,21.404 14.973,34.275z"></path>
                            <path d="m31.128,123.355c1.458,1.658 3.559,2.609 5.767,2.609 2.208,0 4.31-0.951 5.767-2.609l10.496-11.944c2.799-3.185 2.486-8.037-0.699-10.836-2.064-1.813-4.824-2.312-7.275-1.578 1.548-12.871 6.691-24.852 14.972-34.275 2.799-3.185 2.486-8.037-0.699-10.836-3.186-2.799-8.037-2.485-10.836,0.699-10.826,12.32-17.36,28.056-18.923,44.825-2.688-1.277-5.991-0.924-8.368,1.165-3.185,2.799-3.498,7.651-0.699,10.836l10.497,11.944z"></path>
                            <path d="m154.621,69.633c-0.647-0.705-1.594-1.06-2.551-1.06h-7.14c-0.957,0-1.904,0.355-2.551,1.06-1.002,1.092-1.147,2.669-0.436,3.902l3.817,5.754-1.787,15.073 3.518,9.36c0.343,0.941 1.674,0.941 2.017,0l3.518-9.36-1.787-15.073 3.817-5.754c0.712-1.233 0.567-2.81-0.435-3.902z"></path>
                        </g>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 font-semibold">Penjadwalan</p>
        </a>
        <a href="/logout" class="flex items-center justify-start px-5 gap-3 bg-slate-100 bg-opacity-50 h-8 hover:bg-opacity-100 rounded-full mb-3">
            <div class="flex justify-center items-center">
                <svg viewBox="0 0 24 24" class="text-blue-800 w-5 h-5" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M16.125 12C16.125 11.5858 15.7892 11.25 15.375 11.25L4.40244 11.25L6.36309 9.56944C6.67759 9.29988 6.71401 8.8264 6.44444 8.51191C6.17488 8.19741 5.7014 8.16099 5.38691 8.43056L1.88691 11.4306C1.72067 11.573 1.625 11.7811 1.625 12C1.625 12.2189 1.72067 12.427 1.88691 12.5694L5.38691 15.5694C5.7014 15.839 6.17488 15.8026 6.44444 15.4881C6.71401 15.1736 6.67759 14.7001 6.36309 14.4306L4.40244 12.75L15.375 12.75C15.7892 12.75 16.125 12.4142 16.125 12Z" fill="#3359d7"></path>
                        <path d="M9.375 8C9.375 8.70219 9.375 9.05329 9.54351 9.3055C9.61648 9.41471 9.71025 9.50848 9.81946 9.58145C10.0717 9.74996 10.4228 9.74996 11.125 9.74996L15.375 9.74996C16.6176 9.74996 17.625 10.7573 17.625 12C17.625 13.2426 16.6176 14.25 15.375 14.25L11.125 14.25C10.4228 14.25 10.0716 14.25 9.8194 14.4185C9.71023 14.4915 9.6165 14.5852 9.54355 14.6944C9.375 14.9466 9.375 15.2977 9.375 16C9.375 18.8284 9.375 20.2426 10.2537 21.1213C11.1324 22 12.5464 22 15.3748 22L16.3748 22C19.2032 22 20.6174 22 21.4961 21.1213C22.3748 20.2426 22.3748 18.8284 22.3748 16L22.3748 8C22.3748 5.17158 22.3748 3.75736 21.4961 2.87868C20.6174 2 19.2032 2 16.3748 2L15.3748 2C12.5464 2 11.1324 2 10.2537 2.87868C9.375 3.75736 9.375 5.17157 9.375 8Z" fill="#3359d7"></path>
                    </g>
                </svg>
            </div>
            <p class="p-0 mb-0 text-blue-800 font-semibold">Logout</p>
        </a>
    </div>
</aside>

<script>
    const humburgerMenu = document.getElementById('humburger-menu');
    const crossMenu = document.getElementById('cross-menu');
    const sidebar = document.querySelector('aside');
    const navClose = document.getElementById('nav-close');
    const navOpen = document.getElementById('nav-open');
    humburgerMenu.addEventListener('click', () => {
        sidebar.classList.toggle('w-48');
        humburgerMenu.classList.toggle('hidden');
        crossMenu.classList.remove('hidden');
        navOpen.classList.remove('hidden');
        navClose.classList.add('hidden');
    });

    crossMenu.addEventListener('click', () => {
        sidebar.classList.toggle('w-48');
        crossMenu.classList.toggle('hidden');
        humburgerMenu.classList.remove('hidden');
        navClose.classList.remove('hidden');
        navOpen.classList.add('hidden');
    });
</script>