...
<div class="w-full">
    ...
    <div class="shadow-md rounded-md bg-white w-4/5">
        <form action="/dashboard/pembayaran/update/{{ $pembayaran->id }}" class="p-4" method="post" >
            @csrf
            @method('PUT') 
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nominal" id="nominal" class="w-full border border-blue-600 rounded-md p-2" value="{{ $pembayaran->nominal }}" required disabled>
            </div>

            <div class="mb-3">
                <label for="bulan" class="block">Pembayaran untuk Bulan</label>
                <select name="bulan" id="bulan" class="w-full border border-blue-600 rounded-md p-2" required disabled>
                    @php
                        $months = [
                ...
                <input type="hidden" name="bulan" value="{{ explode('-', $pembayaran->bulan)[0] }}">
            </div>

            <div class="mb-3">
                <label for="tahun" class="block">Tahun Pembayaran</label>
                <select name="tahun" id="tahun" class="w-full border border-blue-600 rounded-md p-2" required disabled>
                    @php
                        $currentYear = date('Y');
                ...
                <input type="hidden" name="tahun" value="{{ explode('-', $pembayaran->bulan)[1] }}">
            </div>

            <div class="mb-3">
                <label for="catatan" class="block">Catatan Admin</label>
                <input type="text" name="catatan" id="catatan" class="w-full border border-blue-600 rounded-md p-2" value="{{ $pembayaran->catatan }}">
            </div>
